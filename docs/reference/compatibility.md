...
| **OpenRouter** | `bridge-openrouter` | Yes | Local hash 768-dim (or `OPENROUTER_EMBED_MODEL`) | Set `BRIDGE_MODE=openrouter` + `OPENROUTER_API_KEY`; set `BRIDGE_AUTH_TOKEN` for paid backends |
